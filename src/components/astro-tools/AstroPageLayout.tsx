...
function Sidebar({ currentHref }: { currentHref: string }) {
  ...
  const [downloadState, setDownloadState] = useState<"idle" | "opening" | "opened">("idle");

  const handleDownload = () => {
    setDownloadState("opening");
    window.open("https://www.astroved.com/apphome.aspx", "_blank", "noopener,noreferrer");
    window.setTimeout(() => setDownloadState("opened"), 500);
    ...
    <aside className="flex flex-col gap-5 lg:sticky lg:top-20">
      ...
      >
        ...
        >
          ...
          {downloadState === "opening" ? "Opening..." : downloadState === "opened" ? "Opened" : "Download App"}
        </button>
      </div>
