...
export default function TempleDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (loading) return <div className="min-h-screen bg-white flex justify-center items-center"><Navbar /><div className="animate-spin h-9 w-9 border-4 border-[#f47820] border-t-transparent rounded-full mt-20" /></div>;
  ...
  const Tab = ({ id, label, ref: tabRef }: { id: string; label: string; ref: React.RefObject<HTMLDivElement | null> }) => (
    <button onClick={() => scrollTo(tabRef, id)}
      className={`px-6 py-4 text-[17px] font-semibold border-b-2 transition-all ${activeTab === id ? "border-[#f47820] text-[#f47820]" : "border-transparent text-gray-500 hover:text-gray-800"}`}>
      {label}
    </button>
  );
      ...
      <div className="max-w-[980px] mx-auto pb-20">
        ...
        {showGallery && (
          <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center" onClick={() => setShowGallery(false)}>
            <button className="absolute top-5 right-6 text-white text-3xl font-bold z-50" onClick={() => setShowGallery(false)}>✕</button>
        ...
        <div className="bg-white px-5 pt-6 pb-5 border-b border-gray-100 text-center">
          ...
          <p className="text-[15px] text-[#888] flex items-center justify-center gap-2 flex-wrap">
            ...
            <span className="flex items-center gap-1">
              <svg className="w-4 h-4 text-[#f47820] flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clipRule="evenodd" /></svg>
        ...
        <div className="px-5">
          ...
          <div className="flex justify-center gap-14 py-6 border-b border-gray-200">
            <a href="/puja" className="flex flex-col items-center gap-2 hover:opacity-85 active:scale-95 transition-all cursor-pointer">
              <div className="w-14 h-14 rounded-[14px] bg-gradient-to-br from-[#f47820] to-[#e05c00] flex items-center justify-center shadow-md">
          ...
          {/* Section Nav — inline sticky, sits below action buttons */}
          <div className="sticky top-0 z-40 bg-white border-b border-gray-200 shadow-sm -mx-5 px-8 mt-0">
              ...
              <ul className="space-y-2">
                {temple.festivals.filter(Boolean).map((f: string, i: number) => (
                  <li key={i} className="text-[15px] text-[#555] flex items-start gap-2">
                    <span className="text-[#f47820] font-bold mt-0.5">•</span>{f}
                  </li>
                ))}
              </ul>
            </Sec>
          ...
          {temple.howToReach && Object.values(temple.howToReach).some(v => v) && (
            <Sec title="Travel Details" sRef={locationRef}>
              ...
              {temple.coordinates?.latitude && (
                <div className="rounded-lg overflow-hidden mb-5 h-[280px]">
              ...
              <div className="space-y-2">
                {[
                  { key: "air", label: "Airplane", val: temple.howToReach.byAir, icon: (
                    <svg className="w-5 h-5 text-[#f47820]" fill="currentColor" viewBox="0 0 24 24"><path d="M21 16v-2l-8-5V3.5c0-.83-.67-1.5-1.5-1.5S10 2.67 10 3.5V9l-8 5v2l8-2.5V19l-2 1.5V22l3.5-1 3.5 1v-1.5L13 19v-5.5l8 2.5z"/></svg>
                  )},
                  { key: "train", label: "Train", val: temple.howToReach.byTrain, icon: (
                    <svg className="w-5 h-5 text-[#f47820]" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2c-4 0-8 .5-8 4v9.5C4 17.43 5.57 19 7.5 19L6 20.5v.5h2l2-2h4l2 2h2v-.5L16.5 19c1.93 0 3.5-1.57 3.5-3.5V6c0-3.5-4-4-8-4zm0 2c3.51 0 5.44.49 5.93 1H6.07C6.56 4.49 8.49 4 12 4zM6 7h5v3H6V7zm11 0v3h-5V7h5zm-1 9H8c-.55 0-1-.45-1-1s.45-1 1-1h8c.55 0 1 .45 1 1s-.45 1-1 1z"/></svg>
                  )},
                  { key: "road", label: "Road", val: temple.howToReach.byRoad, icon: (
                    <svg className="w-5 h-5 text-[#f47820]" fill="currentColor" viewBox="0 0 24 24"><path d="M17.66 11.2c-.23-.3-.51-.56-.77-.82-.67-.6-1.43-1.03-2.07-1.66C13.33 7.26 13 4.85 13.95 3c-.95.23-1.78.75-2.49 1.32-2.59 2.08-3.61 5.75-2.39 8.9.04.1.08.2.08.33 0 .22-.15.42-.35.5-.23.1-.47.04-.66-.12a.58.58 0 01-.14-.17c-1.13-1.43-1.31-3.48-.55-5.12C5.78 10 4.87 12.3 5 14.47c.06.5.12 1 .29 1.5.14.6.41 1.2.71 1.73 1.08 1.73 2.95 2.97 4.96 3.22 2.14.27 4.43-.12 6.07-1.6 1.83-1.66 2.47-4.32 1.53-6.6l-.13-.21-.67-.71zM11.71 19c-1.78-.01-3.22-1.4-3.22-3.14 0-1.62 1.05-2.76 2.81-3.12 1.77-.36 3.6-1.21 4.62-2.58.39 1.29.19 2.75-.28 4-.48 1.23-1.4 2.31-2.52 2.96-.51.28-1.32.88-1.41.88z"/></svg>
                  )},
          ...
          <div className="border-t border-gray-200 pt-6 pb-8">
            ...
            <div className="flex items-center gap-7">
              ...
              <a href="#" className="flex items-center gap-2 hover:opacity-80 transition-opacity">
                ...
                <span className="text-[14px] font-semibold" style={{fontStyle:"italic",background:"linear-gradient(45deg,#f09433,#dc2743,#bc1888)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>Instagram</span>
              </a>
              {/* Facebook */}
              <a href="#" className="flex items-center gap-2 hover:opacity-80 transition-opacity">
          ...
          {/* Orange footer bar */}
          <div className="h-2 bg-[#f47820] -mx-5 rounded-b" />
        </div>
      </div>
    </main>
  );
...
function TimingCard({ icon, label, time }: { icon: "calendar" | "bell"; label: string; time: string }) {
  return (
    <div className="bg-[#f7f7f7] rounded-xl p-4 flex items-start gap-3">
      <div className="w-10 h-10 bg-[#f47820] rounded-lg flex items-center justify-center flex-shrink-0">
        {icon === "calendar"
